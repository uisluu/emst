...
*/
    ...
    class TPDF_Antitetanuses extends FPDFEx
    {
        function TPDF_Antitetanuses()
        {
        ...
        function Header()
        {
            ...
            if ( $this->PageNo() == 1 )
            {
                $vX = $this->GetX();
                $vY = $this->GetY();
                $this->Cell($vWidth, $vHeight, iconv('utf-8', 'cp1251','Журнал прививок'), '', 0, 'C');
                $this->SetX($vX);
                ...
                $vRowData= array( iconv('utf-8', 'cp1251','№'),
                                  ...
                                  iconv('utf-8', 'cp1251','Прививка'),
                                  iconv('utf-8', 'cp1251','Серия'));
                $this->OutTableRow($vHeight, $vRowData, 'C');
            }
            else
            {
                $this->Cell($vWidth, $vHeight,iconv('utf-8', 'cp1251', 'Журнал прививок с ').$this->BegDate.iconv('utf-8', 'cp1251',' г. по ').$this->EndDate.iconv('utf-8', 'cp1251',' г. / стр.').$this->PageNo(), '', 0, 'R');
                $this->Ln($vHeight*2);
            }
...
// =======================================================================
    ...
    $vDoc = new TPDF_Antitetanuses;
    $vDoc->Render($_GET);

    $vDoc->Output('Antitetanuses.pdf', 'I');
?>
