  ...
  #####################################################################
    ...
    class TEditor extends HTML_QuickFormEx
    {
        function TEditor($AId='')
        {
            ...
            if ( !empty($AId) )
            {
                $vRecord = $vDB->GetById('rb_vistit_targets', $AId);
                if (  !is_array($vRecord) )
                   $vRecord = array();
            }
            else
            ...
            $this->addElement('header',   'header',          'Кабинет');
...
//            $vId = $vValues['id'];
            ...
            $vResult = $vDB->InsertOrUpdateById('rb_vistit_targets', $vRecord);
...
//       var_dump($vDB);
    ...
    if ( !$vForm->Save() )
    {
        ...
        $vTemplate->compile('refs/vistit_target_edit.html');
        $vTemplate->outputObject($vView);
    }
    else
    {
        RedirectToList('vistit_targets.html');
    }
