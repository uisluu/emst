...
*/
    ...
    class TData extends TBaseView
    {
        function GetTable()
        {
            ...
            $vTab =& new TTable('rb_insurance_companies', '*', '', 'name', 'id');
            $vTab->AddColumn('name',      'Наименование');
            $vTab->AddRowAction('изменить', 'insurance_company_edit.html?id=');
            $vTab->AddTableAction('новая запись',  'insurance_company_edit.html');
...
//  $vView->form = $vRenderer->toObject();
    $vTemplate->compile('refs/insurance_companies.html');
