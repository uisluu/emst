  ...
  #####################################################################
    ...
    class TEditor extends HTML_QuickFormEx
    {
        function TEditor($AId='')
        {
            ...
            $this->addElement('header',   'header',          'Тип травмы');
...
//            $vId = $vValues['id'];
            ...
            $vResult = $vDB->InsertOrUpdateById('rb_trauma_types', $vRecord);
...
//       var_dump($vDB);
    ...
    if ( !$vForm->Save() )
    {
        ...
        $vTemplate->compile('refs/trauma_type_edit.html');
        $vTemplate->outputObject($vView);
    }
    else
    {
        RedirectToList('trauma_types.html');
    }
