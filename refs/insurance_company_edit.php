  ...
  #####################################################################
    ...
    class TEditor extends HTML_QuickFormEx
    {
        function TEditor($AId='')
        {
            ...
            if ( !empty($AId) )
            {
                $vRecord = $vDB->GetById('rb_insurance_companies', $AId);
                if (  !is_array($vRecord) )
                   $vRecord = array();
            }
            else
            ...
            $this->addElement('header',   'header',          'Страховая компания');
...
//            $vId = $vValues['id'];
            ...
            $vResult = $vDB->InsertOrUpdateById('rb_insurance_companies', $vRecord);
...
//       var_dump($vDB);
    ...
    if ( !$vForm->Save() )
    {
        ...
        $vTemplate->compile('refs/insurance_company_edit.html');
        $vTemplate->outputObject($vView);
    }
    else
    {
        RedirectToList('insurance_companies.html');
    }
