  ...
  #####################################################################
    ...
    class TEditor extends HTML_QuickFormEx
    {
        function TEditor($AId='')
        {
            ...
            $this->addElement('header',   'header',          'Прививка от столбняка');
...
//            $vId = $vValues['id'];
            ...
            $vResult = $vDB->InsertOrUpdateById('rb_antitetanus', $vRecord);
...
//       var_dump($vDB);
    ...
    if ( !$vForm->Save() )
    {
        ...
        $vTemplate->compile('refs/antitetanus_edit.html');
        $vTemplate->outputObject($vView);
    }
    else
    {
        RedirectToList('antitetanus.html');
    }
