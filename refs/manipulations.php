...
*/
    ...
    class TData extends TBaseView
    {
        function GetTable()
        {
            ...
            $vTab =& new TTable('rb_manipulations', '*', '', 'name', 'id');
            $vTab->AddColumn('name', 'Наименование');
            $vTab->AddBoolColumn('is_plaster', 'В журнал гипс');
            $vTab->AddRowAction('изменить', 'manipulation_edit.html?id=');
            $vTab->AddTableAction('новая запись',  'manipulation_edit.html');
...
//  $vView->form = $vRenderer->toObject();
    $vTemplate->compile('refs/manipulations.html');
