  ...
  #####################################################################
    ...
    class TEditor extends HTML_QuickFormEx
    {
        function TEditor($AId='')
        {
            ...
            $this->addElement('header',   'header',          'Динамика');
...
//            $vId = $vValues['id'];
            ...
            $vResult = $vDB->InsertOrUpdateById('rb_dynamics', $vRecord);
...
//       var_dump($vDB);
    ...
    if ( !$vForm->Save() )
    {
        ...
        $vTemplate->compile('refs/dynamic_edit.html');
        $vTemplate->outputObject($vView);
    }
    else
    {
        RedirectToList('dynamics.html');
    }
